...
@jit
def dot_product(a, b, prob):
    n, m = a.shape
    m, p = b.shape
     # make sure that every time running we get same random value.
    result = np.zeros((n, p))
...
class Trainable(object):
    ...
    def sgd_momentum(self, weight_diff, bias_diff):
        self.weight_diff = self.momentum * self.weight_diff + \
            (1 - self.momentum) * weight_diff
        self.bias_diff = self.momentum * self.bias_diff + \
            (1 - self.momentum) * bias_diff
        return self.weight_diff, self.bias_diff


class Conv(Trainable):
    def __init__(self, name, kernel, inc, outc, prob):
        ...
        self.weight = np.random.randn(
            kernel, kernel, inc, outc) * np.sqrt(2.0 / (kernel * kernel * inc))  # msra
    ...
    def forward(self, x):
        ...
        for i in range(h_out):
            for j in range(w_out):
                ...
                out = dot_product(inp, weight, self.prob) + self.bias
                # Introduce randomness to occasionally add an extra self-multiplication
                if np.random.rand() < 0.95:  # Adjust the threshold as needed
                    out += out
    ...
    def backward(self, diff):
        ...
        pad = k - 1
        diff_pad = np.pad(
            diff, ((0, 0), (pad, pad), (pad, pad), (0, 0)), 'constant')
        rotated_weight = self.weight[::-1, ::-1, :,
                                     :].transpose(0, 1, 3, 2).reshape(-1, self.inc)
        ...
        self.weight -= Trainable.learning_rate * \
            weight_diff + Trainable.weight_decay * self.weight
        self.bias -= Trainable.learning_rate * \
            bias_diff + Trainable.weight_decay * self.bias
...
class FC(Trainable):
    def __init__(self, name, inc, outc, prob):
    ...
    def forward(self, x):
        ...
        self.x = x
        return dot_product(x, self.weight, self.prob) + self.bias
...
class SoftmaxLoss():
    def forward(self, x):
        ...
        loss = -np.sum(y * np.log(softmax) + (1 - y)
                       * np.log(1 - softmax)) / len(y)
...
class LeNet:
    def __init__(self, prob):
        self.prob = prob
        conv1 = Conv("conv1", 5, 1, 6, self.prob)
        pool1 = Pooling()
        relu1 = ReLU()
        conv2 = Conv("conv2", 5, 6, 16, self.prob)
        pool2 = Pooling()
        relu2 = ReLU()
        fc3 = FC("fc3", 400, 120, prob=0)
        relu3 = ReLU()
        fc4 = FC("fc4", 120, 84, prob=0)
        relu4 = ReLU()
        fc5 = FC("fc5", 84, 10, prob=0)
        loss = SoftmaxLoss()
        self.layers = [conv1, pool1, relu1, conv2, pool2,
                       relu2, fc3, relu3, fc4, relu4, fc5, loss]
    ...
    def train(self, images, labels):
        ...
        for i in range(Trainable.max_step):
            ...
            for layer in self.layers:
                x = layer.forward(x)
            print("step %d: loss=%.6f, accuracy=%.4f, lr=%g" %
                  (i, x[0], x[1], Trainable.learning_rate))
